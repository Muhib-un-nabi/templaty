...
import { addCoupon, setLoading } from '../../../../redux/coupons/action';

const Index = ({ visible, setVisible, loading, addCoupon, setLoading }) => {
  const [name, setName] = useState();
  const handelAddSnippet = async (e) => {
    try {
      e.preventDefault();
      await setLoading();
      await addCoupon({ name });
...
export default connect(mapStateToProps, { addCoupon, setLoading })(Index);
